...
describe('MONITORS_QUERY_KEYS', () => {
  it('builds the "all" key', () => {
    expect(MONITORS_QUERY_KEYS.all).toEqual(['monitors'])
  })

  it('builds the list key', () => {
    expect(MONITORS_QUERY_KEYS.list()).toEqual(['monitors', 'list'])
  })

  it('builds the details key', () => {
    expect(MONITORS_QUERY_KEYS.details()).toEqual(['monitors', 'detail'])
  })

  it('builds a detail key scoped by monitorId', () => {
    expect(MONITORS_QUERY_KEYS.detail('mon-1')).toEqual(['monitors', 'detail', 'mon-1'])
  })

  it('keeps list and detail keys distinct', () => {
    expect(MONITORS_QUERY_KEYS.list()).not.toEqual(MONITORS_QUERY_KEYS.detail('mon-1'))
  })
})
...
describe('useUpdateMonitor', () => {
  ...
  it('optimistically updates the list and detail caches before the mutation resolves', async () => {
    ...
    queryClient.setQueryData(MONITORS_QUERY_KEYS.list(), { monitors: existingList })
    queryClient.setQueryData(MONITORS_QUERY_KEYS.detail('mon-1'), existingDetail)
    ...
    await waitFor(() => {
      const list = queryClient.getQueryData<UserMonitors>(MONITORS_QUERY_KEYS.list())?.monitors
      expect(list).toBeDefined()
      expect(list?.length).toBeGreaterThan(0)
      expect(list?.[0]?.name).toBe('New Name')
    })

    const detail = queryClient.getQueryData<UserMonitor>(MONITORS_QUERY_KEYS.detail('mon-1'))
  ...
  it('rolls back list and detail caches on error', async () => {
    ...
    queryClient.setQueryData(MONITORS_QUERY_KEYS.list(), existingList)
    queryClient.setQueryData(MONITORS_QUERY_KEYS.detail('mon-1'), existingDetail)
    ...
    const list = queryClient.getQueryData<UserMonitor[]>(MONITORS_QUERY_KEYS.list())
    const detail = queryClient.getQueryData<UserMonitor>(MONITORS_QUERY_KEYS.detail('mon-1'))
  ...
  it('invalidates list and detail queries on settle (success)', async () => {
    ...
    expect(invalidateSpy).toHaveBeenCalledWith({ queryKey: MONITORS_QUERY_KEYS.list() })
    expect(invalidateSpy).toHaveBeenCalledWith({
      queryKey: MONITORS_QUERY_KEYS.detail('mon-1'),
    })
  })
  ...
  it('handles a missing previous list gracefully (defaults to empty array)', async () => {
    ...
    const list = queryClient.getQueryData(MONITORS_QUERY_KEYS.list())
