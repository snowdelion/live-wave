...
describe('AnalyticsService', () => {
  ...
  describe('getOverview', () => {
    it('throws NotFoundException when monitor does not exist', async () => {
      ...
      await expect(service.getOverview('client-1', 'monitor-1')).rejects.toThrow(NotFoundException)
    })

    it('throws NotFoundException when monitor belongs to a different client', async () => {
      prisma.monitor.findUnique.mockResolvedValue({ clientId: 'other-client', name: 'X' })
    ...
    it('returns a correctly shaped overview', async () => {
      ...
      const result = await service.getOverview('client-1', 'monitor-1', 7)
    ...
    it('sets averageResponseTime to null when uptime row is missing', async () => {
      ...
      const result = await service.getOverview('client-1', 'monitor-1')
  ...
  describe('getIncidents', () => {
    ...
    it('throws NotFoundException when monitor does not exist', async () => {
      ...
      await expect(service.getIncidents('client-1', 'monitor-1', startDate)).rejects.toThrow(
        NotFoundException,
      )
    })

    it('throws NotFoundException when monitor belongs to a different client', async () => {
      prisma.monitor.findUnique.mockResolvedValue({ clientId: 'other-client' })
    ...
    it('returns incidents list and total count', async () => {
      prisma.monitor.findUnique.mockResolvedValue({ clientId: 'client-1' })
      prisma.$queryRaw
        .mockResolvedValueOnce(makeIncidentRaw())
        .mockResolvedValueOnce([{ count: 1 }])

      const { incidents, total } = await service.getIncidents('client-1', 'monitor-1', startDate)
    ...
    it('maps null durationMs to 0', async () => {
      ...
      const { incidents } = await service.getIncidents('client-1', 'monitor-1', startDate)
    ...
    it('returns 0 total when count row is missing', async () => {
      ...
      const { total } = await service.getIncidents('client-1', 'monitor-1', startDate)
  ...
  describe('getTimeline', () => {
    ...
    it('throws NotFoundException when monitor does not exist', async () => {
      ...
      await expect(service.getTimeline('client-1', 'monitor-1', startDate)).rejects.toThrow(
        NotFoundException,
      )
    })
    ...
    it('returns mapped timeline buckets', async () => {
      ...
      const result = await service.getTimeline('client-1', 'monitor-1', startDate)
    ...
    it('maps null averageResponseTime to null', async () => {
      ...
      const [point] = await service.getTimeline('client-1', 'monitor-1', startDate)
  ...
  describe('bucket size selection', () => {
    ...
    it.each(cases)(
      ...
      async (windowMinutes, expectedBucket) => {
        ...
        await service.getTimeline('client-1', 'monitor-1', startDate)
  ...
  describe('error propagation', () => {
    it('re-throws database errors from getOverview', async () => {
      ...
      await expect(service.getOverview('client-1', 'monitor-1')).rejects.toThrow('DB exploded')
    })
    ...
    it('re-throws database errors from getIncidents', async () => {
      ...
      await expect(service.getIncidents('client-1', 'monitor-1', new Date())).rejects.toThrow(
        'DB exploded',
      )
    })
    ...
    it('re-throws database errors from getTimeline', async () => {
      ...
      await expect(service.getTimeline('client-1', 'monitor-1', new Date())).rejects.toThrow(
        'DB exploded',
      )
    })
    ...
    it('calls logger.error with message and stack when an Error is thrown', async () => {
      ...
      await expect(service.getOverview('client-1', 'monitor-1')).rejects.toThrow('oops')
    ...
    it('calls logger.error with "Unknown error" and no stack for non-Error throws', async () => {
      ...
      await expect(service.getOverview('client-1', 'monitor-1')).rejects.toBe('raw string error')
    ...
    it('handles non-Error throws from getIncidentsList (lines 183-184)', async () => {
      ...
      await expect(service.getIncidents('client-1', 'monitor-1', new Date())).rejects.toBe(42)
